...
module.exports = {
	...
	async execute(interaction) {
		...
		switch(game){
			case "wordle":
				get_wordle(user_id, interaction.guild.id, puzzle_id, interaction);
				break;
			case "connections":
				get_connection(user_id, interaction.guild.id, puzzle_id, interaction);
				break;
			case "mini":
				get_mini(user_id, interaction.guild.id, puzzle_id, interaction);
				break;
			case "strands":
				get_strand(user_id, interaction.guild.id, puzzle_id, interaction);
				break;
		}
	},
};

async function get_wordle(user_id, server_id, puzzle_id, interaction){
...
async function get_connection(user_id, server_id, puzzle_id, interaction){
	const result = await Connections.findOne({where:{user_id: user_id, server_id: server_id, puzzle_id: puzzle_id}});
	if(result){
		return interaction.reply({content:`Puzzle ${result.puzzle_id}: Score: ${result.score} Guesses: ${result.guesses}`,ephemeral:true});
	}
	return interaction.reply({content:`Could not find Connections ${puzzle_id} for that user!`,ephemeral:true});
}
async function get_mini(user_id, server_id, puzzle_id, interaction){
...
async function get_strand(user_id, server_id, puzzle_id, interaction){
